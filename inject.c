...
DWORD WINAPI UninitThread(LPVOID param) {
    Sleep(100);
    MH_Uninitialize();
    return 0;
}
...
int __cdecl hkLuaL_loadstring(lua_State* L, const char* s) {
    if (g_LuaState == NULL && L != NULL) {
        ...
        MH_DisableHook((LPVOID)oLuaL_loadstring_Hook);
        MH_RemoveHook((LPVOID)oLuaL_loadstring_Hook);
		CreateThread(NULL, 0, UninitThread, NULL, 0, NULL);
    }
...
DWORD WINAPI MainThread(LPVOID lpParam) {
    ...
    void* pLoadStr  = (void*)GetProcAddress(hLua, "luaL_loadstring");

    if (!luaL_loadstring || !lua_vpcall || !lua_newthread || !lua_settop || !luaL_ref || !luaL_unref || !pLoadStr) {
        MessageBoxW(NULL, L"Không lấy được hàm Lua!", L"DLL Inject", MB_OK | MB_ICONERROR);
        return 0;
    }

    if (MH_Initialize() == MH_OK) {
        if (MH_CreateHook(pLoadStr, hkLuaL_loadstring, (LPVOID*)&oLuaL_loadstring_Hook) == MH_OK) {
            MH_EnableHook(pLoadStr);
        }
    }
